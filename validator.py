...
class ApigeeValidator():
    ...
    def validate_env_targetservers(self, targetServers):
        ...
        for targetServer in targetServers.keys():
            ...
            obj['importable'], obj['reason'] = self.validate_env_targetserver_resource(targetServers[targetServer])  # noqa
    ...
    def validate_env_targetserver_resource(self, targetservers):
        ...
        for key in targetservers_mapping.keys():
            if targetservers[key] in targetservers_mapping[key]['invalid_values'].keys():  # noqa
                errors.append({
                    'key': key,
                    'error_msg': targetservers_mapping[key]['invalid_values'][targetservers[key]],  # noqa
                })
    ...
    def validate_env_resourcefiles(self, resourcefiles):
        ...
        for resourcefile in resourcefiles.keys():
            ...
            obj['importable'], obj['reason'] = self.validate_env_resourcefile_resource(resourcefiles[resourcefile])  # noqa
    ...
    def validate_env_resourcefile_resource(self, metadata):
        ...
        for key in resourcefiles_mapping.keys():
            if metadata[key] in resourcefiles_mapping[key]['invalid_values'].keys():  # noqa
                errors.append({
                    'key': key,
                    'error_msg': resourcefiles_mapping[key]['invalid_values'][metadata[key]],  # noqa
                })
    ...
    def validate_proxy_bundles(self, export_dir):
        ...
        for each_api_type in ['apis', 'sharedflows']:
            for proxy_bundle in list_dir(export_dir):
                each_validation = self.validate_proxy(export_dir, each_api_type, proxy_bundle)  # noqa
    ...
    def validate_proxy(self, export_dir, each_api_type, proxy_bundle):
        ...
        if 'error' in validation_response:
            obj['name'] = api_name
            obj['importable'], obj['reason'] = False,validation_response['error'].get('details','ERROR')  # noqa
        else:
    ...
    def validate_env_flowhooks(self, env, flowhooks):
        ...
        for flowhook in flowhooks.keys():
            ...
            obj['name'] = flowhook
            obj['importable'], obj['reason'] = self.validate_env_flowhooks_resource(env, flowhooks[flowhook])  # noqa
    ...
    def validate_env_flowhooks_resource(self, env, flowhook):
        ...
        if "sharedFlow" in flowhook:
            envSharedFlowDeployment = self.xorhybrid.get_env_object(env, "sharedflows", flowhook["sharedFlow"]+"/deployments")  # noqa
            if "deployments" in envSharedFlowDeployment and len(envSharedFlowDeployment["deployments"]) == 0:  # noqa
                errors.append({
                    'key': "sharedFlow",
                    'error_msg': f"Flowhook sharedflow - {flowhook['sharedFlow']} is not present in APIGEE X environment {env}",  # noqa
                })
